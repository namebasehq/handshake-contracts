...
import "mocks/MockTextResolver.sol";
...
contract TestHandshakeTld is Test {
    ...
    function testGetMetadataFromSld() public {
        MockHandshakeSld nft = new MockHandshakeSld();
        MockHandshakeTld tld = new MockHandshakeTld();
        MockEmptySldRegistrationManager sldManager = new MockEmptySldRegistrationManager();
        nft.setParent("domain");
        nft.setExpiry(1234567890);
        SldMetadataService metadata = new SldMetadataService(nft, tld, sldManager, "#000000");

        bytes32 namehash = Namehash.getTldNamehash("testing");

        nft.setName(namehash, "handshake.domain");

        sldManager.setRenewalPrice(666);

        string memory uri = metadata.tokenURI(namehash);

        assertEq(
            keccak256(abi.encodePacked(uri)),
            0xcee6351f9a553402261488794c02362acf90e44aa3b4507b6597878b19e86bfd
        );
    ...
    function testGetMetadataFromSldDefaultTldImage() public {
        MockHandshakeSld nft = new MockHandshakeSld();
        MockHandshakeTld tld = new MockHandshakeTld();
        MockEmptySldRegistrationManager sldManager = new MockEmptySldRegistrationManager();
        nft.setParent("domain");
        ...
        MockTextResolver resolver = new MockTextResolver(
            ...
            HandshakeNft(address(tld))
        );

        bytes32 namehash = Namehash.getTldNamehash("testing");
        bytes32 parentHash = Namehash.getTldNamehash("domain");

        nft.setNamehashToParentMap(namehash, parentHash);

        tld.setResolver(parentHash, IResolver(address(resolver)));

        string memory key = "avatar";
        string memory value = "this_is_url";

        resolver.setText(parentHash, key, value);

        nft.setName(namehash, "handshake.domain");

        sldManager.setRenewalPrice(420);
        ...
        */
        ...
        assertEq(
            keccak256(abi.encodePacked(uri)),
            0x9f9a6f6329da8714f8252e5b3ab1ba7fd5483dbc8f02d0ae308192ef2f52ccbf
        );
    }

    function testGetMetadataFromSldDefaultSldImage() public {
        MockHandshakeSld nft = new MockHandshakeSld();
        MockHandshakeTld tld = new MockHandshakeTld();
        MockEmptySldRegistrationManager sldManager = new MockEmptySldRegistrationManager();
        nft.setParent("domain");
        ...
        string memory value2 = "this_is_url2";

        resolver.setText(parentHash, key, value);
        resolver.setText(namehash, key, value2);
        ...
        */
        ...
        assertEq(
            keccak256(abi.encodePacked(uri)),
            0x2a95a2e1a1643d0bcb22025444c52f26ef8da316dbeae03851f6d865a4af1420
        );
    }

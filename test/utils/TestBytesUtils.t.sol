...
contract TestBytesUtils is Test {
    ...
    function testSubstring() public pure {
        ...
        require(
            keccak256(bytes("hello".substring(0, 4))) == keccak256(bytes("hell")),
            "Copy substring"
        );
        require(
            keccak256(bytes("hello".substring(1, 4))) == keccak256(bytes("ello")),
            "Copy substring"
        );
        require(
            keccak256(bytes("hello".substring(0, 5))) == keccak256(bytes("hello")),
            "Copy whole string"
        );
    }

    function testReadUint8() public pure {
        require(uint256("a".readUint8(0)) == 0x61, "a == 0x61");
        require(uint256("ba".readUint8(1)) == 0x61, "a == 0x61");
    }

    function testReadUint16() public pure {
        require(uint256("abc".readUint16(1)) == 0x6263, "Read uint 16");
    }

    function testReadUint32() public pure {
        require(uint256("abcde".readUint32(1)) == 0x62636465, "Read uint 32");
    }

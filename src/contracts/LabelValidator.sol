...
contract LabelValidator is ILabelValidator {
    ...
    /**
     * Currently using Handshake/DNS validation rules -- except that we're not allowing punycode or underscores for now.
     * We have chosen to limit all names to alphanumeric ASCII to start, in order to avoid issues with EPP and existing registrars.
     * In the future we may enable full Unicode, possibly with on-chain punycoding for backwards compatibility.

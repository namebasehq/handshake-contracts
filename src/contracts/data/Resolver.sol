...
import "interfaces/data/IResolver.sol";
...
contract Resolver is IResolver {
    ...
    function setDnsRecord(bytes32 _namehash, bytes32 _name, uint16 _resource) external isDomainContract returns (bytes memory){
    ...
    function setPublicKey(bytes32 _namehash, bytes32 x, bytes32 y) external isDomainContract {

    }

    function setText(bytes32 _namehash, string calldata key, string calldata value) external isDomainContract {

    }

    modifier isDomainContract() {
        require(msg.sender == SldAddress 
            || msg.sender == TldAddress
            , "NOT AUTHORISED");
        _;
    }
